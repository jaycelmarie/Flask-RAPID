...
class Detector():
    ...
    def detect_one(self, **kwargs):
        ...
        if kwargs.get('visualize', False):
            ...
            np_img2 = cv2.putText(np_img, str("RAPiD"),
                    (200, 250), font, 
                    3, (0,255,0), 
                    2, cv2.LINE_AA) 
            img_rgb = cv2.cvtColor(np_img2, cv2.COLOR_BGR2RGB)
            save_path = './uploads'
       	    cv2.imwrite(os.path.join(save_path,'savedimg' + '_' + str(random.randint(1,1000)) + '.jpg'), img_rgb)
            print('==== Image has been saved locally! ====')
